...
export class App implements OnInit, OnDestroy {
  ...
  openAvatarPanel(playerId: string) {
    this.avatarPanelPlayerId = playerId;
    this.showAvatarPanel = true;
  }

  closeAvatarPanel() {
    this.showAvatarPanel = false;
    this.avatarPanelPlayerId = '';
  }

  setDicebearAvatar(playerId: string) {
  this.apiService.setAvatarStyle(playerId, this.selectedDicebearStyle).subscribe({
    next: () => {
      this.loadPlayers();
      this.closeAvatarPanel();
    },
    error: (err) => alert(this.formatError(err, 'Could not set avatar style.'))
  });
}

  uploadLocalAvatar(playerId: string, event: Event) {
    ...
    upload$.subscribe({
      ...
      error: (err) => alert(this.formatError(err, 'Could not upload avatar.'))
    });
  }

  dicebearPreviewUrl(style: string, playerId: string): string {
    return `https://api.dicebear.com/9.x/${style}/svg?seed=${playerId || 'preview'}`;
  }
